...
import { type RawTerm, looksLikeTerm, termToString } from './rdf/rdf-model.js';
import { makeTermMap } from './common.js';
import {
  Match, ResourceShape, LiteralShape, Vocabulary, TypedVocabulary, ValueMapper, NamedMapper,
} from './shapes.js';
import { rampjs, rdf, xsd } from './vocabulary.js';
...
export function mapAsTerm<T extends Term>(
  factory: DataFactory
): NamedMapper<RawTerm<T>, T> {
  return {
    type: factory.namedNode(rampjs.MapAsTerm),
...
export function mapAsString<T extends NamedNode | BlankNode | Literal>(
  factory: DataFactory
): NamedMapper<RawTerm<T>, string> {
  return {
    type: factory.namedNode(rampjs.MapAsString),
    map: (value, shape) => {
...
export function mapAsNumber(factory: DataFactory): NamedMapper<RawTerm<Literal>, number> {
  return {
    type: factory.namedNode(rampjs.MapAsNumber),
    map: (value, shape) => {
...
export function mapAsBoolean(factory: DataFactory): NamedMapper<RawTerm<Literal>, boolean> {
  return {
    type: factory.namedNode(rampjs.MapAsBoolean),
    map: (value, shape) => {
