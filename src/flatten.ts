import type { DataFactory, BlankNode, Literal, NamedNode, Quad, Term } from '@rdfjs/types';
import type { HashMap } from '@reactodia/hashmap';
...
import {
  ...
  ShapeReference, ValueMapper, Match, getNestedPropertyPath,
} from './shapes.js';
import { SubjectMemo, makeListShapeDefaults, makeTermMap, resolveListShape } from './common.js';
import { RampError, ErrorCode, formatDisplayShape, makeRampError } from './errors.js';
import { ReferenceMatch, synthesizeShape } from './synthesize.js';
import {
...
export function *flatten<S extends Shape>(params: FlattenParams<S>): Iterable<Quad> {
  ...
  const visitor: TransformVisitor<ShapeMatch> = {
    ...
    intoMapItem: (key, value, shape) => {
      if (shape.value) {
        ...
        addRefMatch(refs, shape.value, value);
        const item = synthesizeShape(shape.itemShape, {
          factory: context.factory,
          matches: refs,
          makeError: (code, message) => context.makeError(code, message),
        });
        return new Match([item, shape.itemShape]);
      } else {
        return new Match([value, shape.itemShape]);
      }
    },
    ...
    fromMapItem: (key, value, shape) => undefined,
  ...
  queuedGenerations.push({match: rootMatch});
