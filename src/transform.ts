...
import { assertUnknownShape, matchesTerm } from './common.js';
import {
...
import {
  ...
  LiteralShape, ListShape, MapShape, ShapeID, ValueHole, Match,
} from './shapes.js';
import { synthesizeShape, EMPTY_REF_MATCHES } from './synthesize.js';
...
function transformMap<M>(
  ...
  const itemFrame: StackFrame = {shape: shape.itemShape};
  ...
  for (const key in value) {
    ...
    const valueMatch = context.visitor.fromMapItem(key, valueAtKey, shape);
    if (valueMatch) {
      matches[key] = valueMatch;
    } else {
      const pair = context.visitor.intoMapItem(key, valueAtKey, shape);
      if (!pair) {
        throw context.makeError(
      ...
      const [pairValue, pairShape] = pair.value;
      const itemMatch = transformShape(pairShape, required, pairValue, itemFrame, context);
      if (itemMatch) {
...
function transformRecord<M>(
  ...
  const checkProperties = required || matches.some(m => m.property.kind !== 'transient');
  if (!(
