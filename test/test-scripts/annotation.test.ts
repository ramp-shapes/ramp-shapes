...
import { assertEqual, assertEqualStructural } from '../core/assertions.js';
...
export default (context: TestScriptContext): void => {
  const dataset = Ramp.dataset(readTurtle(
    path.join(import.meta.dirname, 'annotation.ttl')
  ).quads);

  context.defineCase('annotation/roundtrip', async () => {
    ...
    const generatedQuads = Array.from(Ramp.flatten({
      value: matches[0],
    ...
    const expected = readTurtle(
      path.join(import.meta.dirname, 'annotation-roundtrip.ttl'),
      new SequentialDataFactory(Ramp.DefaultDataFactory),
    );
    const generatedTurtle = await quadsToTurtleString(generatedQuads, expected.prefixes);

    assertEqual(
      generatedTurtle,
      expected.turtle,
      'Flatten produced different result graph in a roundtrip'
    );
  });

  context.defineCase('annotation/roundtrip-backwards', async () => {
    ...
    const generatedQuads = Array.from(Ramp.flatten({
      value: matches[0],
      shape: backwardsShape,
      unstable_generateBlankNode: prefix => sequentialFactory.blankNode(),
    }));

    const expected = readTurtle(
      path.join(import.meta.dirname, 'annotation-roundtrip-backwards.ttl'),
      new SequentialDataFactory(Ramp.DefaultDataFactory),
    );
    const generatedTurtle = await quadsToTurtleString(generatedQuads, expected.prefixes);
