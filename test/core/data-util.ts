...
import type {
  ...
  Quad, Quad_Graph, Quad_Object, Quad_Predicate, Quad_Subject, Variable, Term, BaseQuad,
} from '@rdfjs/types';
...
export interface TurtleParseResult {
  turtle: string;
  quads: Quad[];
  prefixes: Record<string, NamedNode>;
}

export function readTurtle(path: string, factory = Ramp.DefaultDataFactory): TurtleParseResult {
  const turtle = fs.readFileSync(path, {encoding: 'utf-8'});
  const parser = new N3.Parser({format: 'turtle', factory, blankNodePrefix: ''});
  const prefixes: Record<string, NamedNode> = {};
  const quads = parser.parse(turtle, null, (prefix, prefixNode) => {
    prefixes[prefix] = prefixNode;
  });
  return {turtle, quads, prefixes};
}
...
export function readTestShapes(filePath: string, root?: Ramp.ShapeID): Ramp.Shape {
  let shapeQuads: Quad[];
  let shapes: Ramp.Shape[];
  try {
    shapeQuads = readTurtle(filePath).quads;
    shapes = Ramp.frameShapes(Ramp.dataset(shapeQuads));
  } catch (err) {
    throw new Error('Failed to read test shapes', {cause: err});
  }
...
export function readTestGraph(filePath: string): Ramp.IndexedDataset {
  try {
    return Ramp.dataset(readTurtle(
      filePath,
      new SequentialDataFactory(Ramp.DefaultDataFactory)
    ).quads);
  } catch (err) {
    throw new Error('Failed to read test graph', {cause: err});
  }
}
